...
void GameBoardQuiz::createWindow() {
    if (!quizWindow.isOpen()) {
        quizWindow.create(sf::VideoMode(1200, 700), "Question", sf::Style::None);
...
void GameBoardQuiz::positionQuestions() {
    ...
    const int windowWidth = static_cast<int>(quizWindow.getSize().x);
    ...
    if (currentQuestion == nullptr) {
        ...
        questionText.setPosition(
            (static_cast<float>(windowWidth) - textBounds.width) / 2,
            80
        );
        return;
    }
    ...
    questionText.setPosition(
        (static_cast<float>(windowWidth) - questionBounds.width) / 2,
        100
    );
    ...
    for (size_t i = 0; i < options.size(); ++i) {
        ...
        float boxX = (static_cast<float>(windowWidth) - optionBoxWidth) / 2;
...
int GameBoardQuiz::getOptionAtPosition(const sf::Vector2i& position) const {
    const sf::Vector2f worldPos = quizWindow.mapPixelToCoords(position);
    for (size_t i = 0; i < optionBoxes.size(); ++i) {
